...
| ../../../../../context/codebase.md | 業務システムのフォルダ対応・連携関係 |
| ../../../../../incidents/ | 過去の障害・調査事例（障害調査時に必ず参照） |
...
コードレビュー・影響調査・バグ調査の依頼を受けた場合、以下の順序で必ず自分でコードを確認すること。
**ユーザーにコードの貼り付けを求めるのは、`src/` が空の場合のみ。**

1. `context/codebase.md` で対象システムのフォルダ名を確認する
2. `systems/{システム名}/README.md` でシステム概要・主要ファイルを確認する
3. `systems/{システム名}/src/` 配下のファイルを直接参照して分析する
4. 連携先システムが関係する場合は、連携先の `systems/` も合わせて参照する
5. `systems/{システム名}/src/` が**存在しない・空の場合のみ**、以下を案内する：
   > 「`systems/{システム名}/src/` に資材が見つかりません。`systems/README.md` の手順でSVNから資材を配置してから再度お試しください。」
